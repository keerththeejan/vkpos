...
class LabelsController extends Controller
{
    ...
    public function preview(Request $request)
    {
        try {
            ...
            $sticker_layout = $request->get('sticker_layout', 'default');
            $layout_map = [
                'default' => 'default',
                'line1' => 'line1',
                'line2' => 'line2',
            ...
            $layout_partial = $layout_map[$sticker_layout] ?? 'default';

            $custom_layout = json_decode($request->get('custom_layout', '[]'), true);
            ...
            if ($request->get('live_preview') && $request->get('live_preview_sample')) {
            ...
            foreach ($product_details_page_wise as $page => $page_products) {
                ...
                $output = view('labels.partials.preview_2')
                            ->with(compact('print', 'page_products', 'business_name', 'barcode_details', 'margin_top', 'margin_left', 'paper_width', 'paper_height', 'is_first', 'is_last', 'factor', 'sticker_layout', 'layout_partial', 'custom_layout'))->render();
                $html .= $output;

                $i++;
            }

            if ($request->get('live_preview')) {
