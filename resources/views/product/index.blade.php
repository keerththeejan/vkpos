...
@section('css')
    <link rel="stylesheet" href="{{ asset('css/products-premium.css?v=' . $asset_v) }}">
@endsection

@section('content')
@php
    $pm_category_count = is_countable($categories) ? count($categories) : 0;
    $pm_brand_count = is_countable($brands) ? count($brands) : 0;
    $pm_unit_count = is_countable($units) ? count($units) : 0;
    $pm_tax_count = is_countable($taxes) ? count($taxes) : 0;
    $pm_location_count = is_countable($business_locations) ? count($business_locations) : 0;
@endphp

<section class="content pm-shell">
    <div class="pm-header">
        <div>
            <h1>@lang('sale.products')</h1>
            <p class="pm-subtitle">@lang('lang_v1.manage_products')</p>
        ...
        <div class="pm-header-actions">
            <button type="button" class="pm-btn pm-btn-ghost" id="pm_dark_mode_toggle" title="Dark Mode" aria-label="Toggle dark mode">
                <i class="fas fa-moon"></i>
            </button>
            <button type="button" class="pm-btn pm-btn-ghost" id="pm_refresh_products" title="Refresh" aria-label="Refresh">
                <i class="fas fa-sync-alt"></i>
            </button>
            @if ($is_admin)
                <a class="pm-btn" href="{{ action([\App\Http\Controllers\ProductController::class, 'downloadExcel']) }}">
                    <i class="fas fa-download"></i> @lang('lang_v1.download_excel')
                </a>
            @endif
            <a class="pm-btn" href="{{ url('/import-products') }}">
                <i class="fas fa-file-import"></i> Import
            </a>
            <a class="pm-btn" href="{{ action([\App\Http\Controllers\LabelsController::class, 'show']) }}">
                <i class="fas fa-barcode"></i> @lang('barcode.labels')
            </a>
            @can('product.create')
                <a class="pm-btn pm-btn-primary" href="{{ action([\App\Http\Controllers\ProductController::class, 'create']) }}">
    ...
    <div class="pm-kpi-grid">
        <div class="pm-kpi tone-blue">
            <div class="pm-kpi-icon"><i class="fas fa-cubes"></i></div>
            <span class="pm-kpi-label">@lang('sale.products')</span>
            <span class="pm-kpi-value" id="pm_kpi_total_products">—</span>
            <span class="pm-kpi-hint">Total in catalog</span>
        </div>
        <div class="pm-kpi tone-green">
            <div class="pm-kpi-icon"><i class="fas fa-filter"></i></div>
            <span class="pm-kpi-label">Filtered</span>
            <span class="pm-kpi-value" id="pm_kpi_filtered_products">—</span>
            <span class="pm-kpi-hint">Matching current filters</span>
        </div>
        <div class="pm-kpi tone-purple">
            <div class="pm-kpi-icon"><i class="fas fa-folder"></i></div>
            <span class="pm-kpi-label">@lang('product.category')</span>
            <span class="pm-kpi-value">{{ $pm_category_count }}</span>
            <span class="pm-kpi-hint">Available categories</span>
        </div>
        <div class="pm-kpi tone-orange">
            <div class="pm-kpi-icon"><i class="fas fa-copyright"></i></div>
            <span class="pm-kpi-label">@lang('product.brand')</span>
            <span class="pm-kpi-value">{{ $pm_brand_count }}</span>
            <span class="pm-kpi-hint">Available brands</span>
        </div>
        <div class="pm-kpi tone-slate">
            <div class="pm-kpi-icon"><i class="fas fa-balance-scale"></i></div>
            <span class="pm-kpi-label">@lang('product.unit')</span>
            <span class="pm-kpi-value">{{ $pm_unit_count }}</span>
            <span class="pm-kpi-hint">Units configured</span>
        </div>
        <div class="pm-kpi tone-red">
            <div class="pm-kpi-icon"><i class="fas fa-percent"></i></div>
            <span class="pm-kpi-label">@lang('product.tax')</span>
            <span class="pm-kpi-value">{{ $pm_tax_count }}</span>
            <span class="pm-kpi-hint">Tax rates</span>
        </div>
        <div class="pm-kpi tone-green">
            <div class="pm-kpi-icon"><i class="fas fa-map-marker-alt"></i></div>
            <span class="pm-kpi-label">@lang('purchase.business_location')</span>
            <span class="pm-kpi-value">{{ $pm_location_count }}</span>
            <span class="pm-kpi-hint">Locations</span>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 pm-filters-wrap">
                ...
                <div class="nav-tabs-custom">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active " id="product_list_tab">
                            <div class="pm-toolbar">
                                <div class="pm-toolbar-left">
                                    <div class="pm-view-toggle" role="group" aria-label="View mode">
                                        <button type="button" id="pm_view_table" class="is-active" title="Table view"><i class="fas fa-table"></i></button>
                                        <button type="button" id="pm_view_cards" title="Card view"><i class="fas fa-th-large"></i></button>
                                    </div>
                                </div>
                                <div class="pm-toolbar-right">
                                    @if ($is_admin)
                                        <a class="pm-btn" href="{{ action([\App\Http\Controllers\ProductController::class, 'downloadExcel']) }}">
                                            <i class="fas fa-file-excel"></i> @lang('lang_v1.download_excel')
                                        </a>
                                    @endif
                                    @can('product.create')
                                        <a class="pm-btn pm-btn-primary" href="{{ action([\App\Http\Controllers\ProductController::class, 'create']) }}">
                            ...
                            <div id="pm_products_card_view" aria-live="polite"></div>
                        </div>
                        @can('stock_report.view')
...
@section('javascript')
    ...
    <script src="{{ asset('js/products-premium-ui.js?v=' . $asset_v) }}"></script>
