...
<div class="table-responsive">
    ...
    @foreach($purchase->purchase_lines as $purchase_line)
        @php
            $line_serials = \App\ProductSerialNumber::where('purchase_line_id', $purchase_line->id)
                ->orderBy('id')
                ->pluck('serial_number')
                ->toArray();
            $pir_row_index = $loop->index;
        @endphp
        <tr class="purchase_product_row" data-enable_sr_no="{{ (int) $purchase_line->product->enable_sr_no }}" data-row_index="{{ $pir_row_index }}" data-product_id="{{ $purchase_line->product_id }}" data-variation_id="{{ $purchase_line->variation_id }}" data-existing-serials="{{ e(json_encode($line_serials)) }}" @if(!empty($purchase_line->purchase_order_line) && !empty($common_settings['enable_purchase_order'])) data-purchase_order_id="{{$purchase_line->purchase_order_line->transaction_id}}" @endif  @if(!empty($purchase_line->purchase_requisition_line) && !empty($common_settings['enable_purchase_requisition'])) data-purchase_requisition_id="{{$purchase_line->purchase_requisition_line->transaction_id}}" @endif>
            ...
            <td>
                ...
                <div class="pir-panel" data-row-index="{{ $pir_row_index }}" style="margin-top:10px;text-align:left;">
                    <div class="pir-head">
                        <strong><i class="fas fa-barcode"></i> IMEI / Serial Receiving</strong>
                        <span class="pir-counters">
                            <span>Required <b class="pir-required">0</b></span>
                            <span>Entered <b class="pir-entered">0</b></span>
                            <span>Remaining <b class="pir-remaining">0</b></span>
                        </span>
                    </div>
                    <div class="pir-progress"><div class="pir-progress-bar" style="width:0%"></div></div>
                    <div class="pir-actions">
                        <button type="button" class="btn btn-xs btn-primary pir-add"><i class="fa fa-plus"></i> Add IMEI</button>
                        <button type="button" class="btn btn-xs btn-default pir-scan"><i class="fa fa-qrcode"></i> Scan</button>
                        <button type="button" class="btn btn-xs btn-default pir-paste"><i class="fa fa-paste"></i> Paste</button>
                        <button type="button" class="btn btn-xs btn-default pir-import"><i class="fa fa-file-excel-o"></i> CSV</button>
                        <button type="button" class="btn btn-xs btn-danger pir-clear"><i class="fa fa-trash"></i> Clear</button>
                        <input type="file" class="pir-file hide" accept=".csv,.txt,.xlsx,.xls">
                    </div>
                    <div class="table-responsive">
                        <table class="table table-condensed table-bordered pir-table" style="margin-bottom:0;background:#fff;">
                            <thead>
                                <tr>
                            ...
                            <tbody class="pir-tbody">
                                @foreach($line_serials as $sn)
                                <tr class="pir-row">
                                    <td class="pir-no"></td>
                                    <td><input type="text" class="form-control input-sm pir-input" name="purchases[{{ $pir_row_index }}][serial_numbers][]" value="{{ $sn }}" maxlength="100" autocomplete="off"></td>
            ...
            <td>
                ...
                $purchase_line->id); !!}
                <input type="hidden" class="enable_sr_no" value="{{ (int) $purchase_line->product->enable_sr_no }}">
                <input type="hidden" class="purchase_row_product_name" value="{{ $purchase_line->product->name }}">
                <input type="hidden" class="purchase_row_index" value="{{ $loop->index }}">
