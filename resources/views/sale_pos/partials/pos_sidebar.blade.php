@php
    $premium_layout = !empty($premium_layout);
    $categories_only = !empty($categories_only);
    $products_only = !empty($products_only);
    $show_categories = !$products_only;
    $show_products = !$categories_only;
@endphp
...
@if ($show_categories)
<div class="row tw-mb-1 @if($premium_layout) pos-cat-list-wrap @endif">
    @if (!empty($categories))
        <div class="@if($premium_layout) col-md-12 @else col-md-6 @endif !tw-px-2" id="product_category_div">
            @if ($premium_layout)
                {{-- Always-visible category cards; keep JS hooks intact --}}
                <div class="pos-cat-list">
                    <div class="pos-cat-card main-category-div main-category no-print is-active"
                        data-value="all" data-parent="0" data-name="{{ __('lang_v1.all_category') }}">
                        <span class="pos-cat-icon"><i class="fas fa-border-all"></i></span>
                        <h4 class="pos-cat-name">@lang('lang_v1.all_category')</h4>
                    </div>

                    @foreach ($categories as $category)
                        <div class="pos-cat-card main-category-div no-print"
                            data-value="{{ $category['id'] }}" data-name="{{ $category['name'] }}" data-parent="1">
                            <span class="pos-cat-icon"><i class="fas fa-folder"></i></span>
                            <h4 class="pos-cat-name">{{ $category['name'] }}</h4>
                            <div class="pos-cat-meta">
                                <button type="button" class="btn btn-xs btn-success main-category" data-value="{{ $category['id'] }}" data-parent="0">{{ __('lang_v1.all') }}</button>
                                @if (!empty($category['sub_categories']))
                                    <button type="button" class="btn btn-xs btn-primary main-category" data-parent="1" data-value="{{ $category['id'] }}" data-name="{{ $category['name'] }}">@lang('pagination.next')</button>
                                @endif
                            </div>
                        </div>
                    @endforeach
                        ...
                        @if (!empty($category['sub_categories']))
                            <div class="{{ $category['id'] }} all-sub-category" style="display: none">
                                @foreach ($category['sub_categories'] as $sc)
                                    @if ($sc['parent_id'] != 0)
                                        <div class="pos-cat-card product_category no-print"
                                            data-value="{{ $sc['id'] }}" data-name="{{ $sc['name'] }}">
                                            <span class="pos-cat-icon"><i class="fas fa-tag"></i></span>
                                            <h4 class="pos-cat-name">{{ $sc['name'] }}</h4>
                                        </div>
                                    @endif
                                @endforeach
                            </div>
                ...
                {{-- Keep drawer markup available for overlay triggers used by pos.js --}}
                <input id="my-drawer-4" type="checkbox" class="tw-dw-drawer-toggle" style="display:none">
                <label for="my-drawer-4" class="tw-dw-drawer-overlay overlay-category" style="display:none"></label>
                <button type="button" class="category-back" style="display:none"><i class="fas fa-chevron-left"></i> @lang('messages.back')</button>
                <div class="category_heading">@lang('category.category')</div>
                <button type="button" class="close-side-bar-category" style="display:none"></button>
            @else
    ...
    @if (!empty($brands))
        <div class="@if($premium_layout) col-md-12 @else col-sm-6 @endif !tw-px-2" id="product_brand_div">
            @if ($premium_layout)
                <div class="pos-cat-list" style="margin-top:10px;">
                    <div class="premium-pos-panel-head" style="padding:8px 0;border:0;background:transparent;">
                        <h4>@lang('brand.brands')</h4>
                    </div>
                    @foreach ($brands as $key => $brand)
                        <div class="pos-cat-card product_brand no-print" data-value="{{ $key }}" data-name="{{ $brand }}">
                            <span class="pos-cat-icon"><i class="fas fa-copyright"></i></span>
                            <h4 class="pos-cat-name">{{ $brand }}</h4>
                        </div>
                    @endforeach
                </div>
                <input id="my-drawer-brand" type="checkbox" class="tw-dw-drawer-toggle" style="display:none">
                <label for="my-drawer-brand" class="tw-dw-drawer-overlay overlay-brand" style="display:none"></label>
                <button type="button" class="close-side-bar-brand" style="display:none"></button>
            @else
                <div class="tw-dw-drawer tw-dw-drawer-end">
    ...
    <div class="col-sm-4 @if (empty($featured_products)) hide @endif" id="feature_product_div">
        <button type="button" class="btn btn-primary btn-flat pos-featured-toggle"
