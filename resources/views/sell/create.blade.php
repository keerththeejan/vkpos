...
@section('css')
<link rel="stylesheet" href="{{ asset('css/sells-create-premium.css?v=' . $asset_v) }}">
@endsection

@section('content')
<!-- Main content -->
<section class="content no-print sc-shell">
	{{-- Sticky Header --}}
	<div class="sc-header" role="banner">
		<div class="sc-header-left">
			<h1>{{ $title }}</h1>
			<p class="sc-subtitle">Customer · products · IMEI · tax · payment · invoice</p>
		...
		<div class="sc-header-actions">
			<button type="button" class="sc-btn sc-btn-ghost" id="sc_dark_mode_toggle" title="Dark Mode" aria-label="Toggle dark mode"><i class="fas fa-moon"></i></button>
			<button type="button" class="sc-btn" id="sc_refresh_btn" title="Refresh"><i class="fas fa-sync-alt"></i> Refresh</button>
			<a class="sc-btn" href="{{ action([\App\Http\Controllers\SellController::class, 'index']) }}"><i class="fas fa-times"></i> Cancel</a>
			<button type="button" class="sc-btn sc-btn-success" id="sc_header_save_print"><i class="fas fa-print"></i> @lang('lang_v1.save_and_print')</button>
			<button type="button" class="sc-btn sc-btn-primary" id="sc_header_save"><i class="fas fa-save"></i> @lang('messages.save')</button>
		</div>
		<div class="sc-header-meta" aria-label="Sale context">
			<div class="sc-meta-item"><span>Invoice #</span><strong id="sc_meta_invoice">Auto</strong></div>
			<div class="sc-meta-item"><span>Location</span><strong id="sc_meta_location">—</strong></div>
			<div class="sc-meta-item"><span>Customer</span><strong id="sc_meta_customer">—</strong></div>
			<div class="sc-meta-item"><span>Date</span><strong id="sc_meta_date">—</strong></div>
			<div class="sc-meta-item"><span>Status</span><strong id="sc_meta_status">—</strong></div>
		</div>
	</div>

	<div class="sc-future-strip" aria-label="Coming soon">
		<span class="sc-chip"><i class="fas fa-barcode"></i> Barcode Scan</span>
		<span class="sc-chip"><i class="fas fa-mobile-alt"></i> IMEI Select</span>
		<span class="sc-chip"><i class="fas fa-qrcode"></i> QR Pay</span>
		<span class="sc-chip"><i class="fas fa-camera"></i> Camera Scan</span>
		<span class="sc-chip"><i class="fas fa-robot"></i> AI Recommend</span>
		<span class="sc-chip muted">UI shell — existing POS/sale logic unchanged</span>
	</div>
...
<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
	...
	<div class="row">
		<div class="col-md-12 col-sm-12">
			@component('components.widget', ['class' => 'box-solid'])
				<div class="sc-section-title"><i class="fas fa-user-tie"></i> Customer &amp; Invoice Information</div>
			...
			@component('components.widget', ['class' => 'box-solid'])
				<div class="sc-section-title"><i class="fas fa-boxes"></i> Products · Search · Cart</div>

				{{-- Product search ABOVE cart so autocomplete is visible (same IDs / attrs as UltimatePOS) --}}
				<div class="row">
					<div class="col-sm-12">
						<div class="form-group" id="sc_product_search_wrap">
							<div class="input-group">
			...
			{{-- IMEI panel placeholder: selection happens via existing product row modal / enable_sr_no flow --}}
			<div class="sc-imei-placeholder" id="sc_imei_placeholder">
				<div class="sc-section-title"><i class="fas fa-barcode"></i> IMEI / Serial Number Selection</div>
				<p class="sc-muted">For IMEI-tracked products, the existing sale row modal lists <strong>Available</strong> serials. Select IMEIs there — quantity and stock rules stay unchanged. Buttons below are UI cues only.</p>
				<div class="sc-imei-actions">
					<button type="button" class="sc-btn" disabled title="Use product row IMEI picker"><i class="fas fa-qrcode"></i> Scan IMEI</button>
					<button type="button" class="sc-btn" disabled title="Use product row IMEI picker"><i class="fas fa-list"></i> Select IMEI</button>
					<button type="button" class="sc-btn" disabled title="Use product row IMEI picker"><i class="fas fa-paste"></i> Paste IMEI</button>
					<button type="button" class="sc-btn" disabled title="Search via product barcode / IMEI in search box"><i class="fas fa-search"></i> Search IMEI</button>
				</div>
			</div>

			@component('components.widget', ['class' => 'box-solid'])
				<div class="sc-section-title"><i class="fas fa-percent"></i> Discount · Tax · Shipping</div>
	...
	<aside class="sc-summary" aria-label="Invoice summary">
		<div class="sc-summary-card">
			<h3>Invoice Summary</h3>
			<div class="sc-kpi-mini">
				<div><span class="k">Items / Qty</span><strong id="sc_kpi_qty">0</strong></div>
				<div><span class="k">Grand Total</span><strong id="sc_kpi_grand">0</strong></div>
			</div>
			<div class="sc-summary-rows">
				<div><span>Line Items</span><strong><span id="sc_sum_items">0</span></strong></div>
				<div><span>Quantity</span><strong><span id="sc_sum_qty">0</span></strong></div>
				<div><span>Subtotal</span><strong><span id="sc_sum_subtotal">0</span></strong></div>
				<div><span>Discount</span><strong><span id="sc_sum_discount">0</span></strong></div>
				<div><span>Tax</span><strong><span id="sc_sum_tax">0</span></strong></div>
				<div class="sc-summary-total"><span>Grand Total</span><strong><span id="sc_sum_grand">0</span></strong></div>
				<div><span>Balance Due</span><strong><span id="sc_sum_balance">0</span></strong></div>
			</div>
			<button type="button" class="sc-btn sc-btn-primary sc-btn-block" id="sc_side_save"><i class="fas fa-save"></i> @lang('messages.save')</button>
			<button type="button" class="sc-btn sc-btn-success sc-btn-block" id="sc_side_print"><i class="fas fa-print"></i> @lang('lang_v1.save_and_print')</button>
			<a class="sc-btn sc-btn-block" href="{{ action([\App\Http\Controllers\SellController::class, 'index']) }}"><i class="fas fa-arrow-left"></i> Back to sales</a>
		</div>
	</aside>
	</div>{{-- /.sc-layout --}}
</section>
...
@section('javascript')
	...
	<script src="{{ asset('js/sells-create-premium-ui.js?v=' . $asset_v) }}"></script>
